<template>
    <div class="company-news">
        ...
        <div class="operation-bar">
            <el-button type="primary" @click="handleAdd">添加新的传统数字村落博物馆</el-button>
        </div>
            ...
            <el-card v-for="news, index in newsList" :key="news.id" class="news-item">
                <template #header>
                    <div class="card-header">
                        <span style="font-weight: bolder;">第 {{ index + 1 }} 个传统数字村落博物馆项目</span>
                        <div class="card-div">
                ...
                <div class="news-content">
                    <img v-if="news.imageUrl" :src="news.imageUrl" class="news-image" alt="传统数字村落博物馆" />
        ...
        <!-- 编辑/添加对话框 -->
        <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑传统数字村落博物馆项目' : '新增传统数字村落博物馆项目'">
            <el-form :model="form" :rules="rules" ref="formRef">
                <el-form-item label="传统数字村落博物馆的简要描述" prop="caseIntroduction">
...
// 验证规则
const rules = {
    caseIntroduction: [{ required: true, message: '请输入传统数字村落博物馆简要描述', trigger: 'blur' }],
}
...
const fetchNews = async () => {
    try {
        ...
        // 过滤出 type 为 "传统数字村落博物馆" 的项目
        newsList.value = data.data.filter(item => item.type === '传统数字村落博物馆').reverse();
        console.log(newsList.value)
    } catch (error) {
        ElMessage.error('获取传统数字村落博物馆列表失败');
    }
}
...
const submitForm = async () => {
    ...
    // 用户对象
    let parameterAdd = { caseIntroduction: form.value.caseIntroduction,type: '传统数字村落博物馆' };
    let parameterEdit = {id:form.value.id, imageUrl: form.value.baseImageURL,caseIntroduction: form.value.caseIntroduction,type: '传统数字村落博物馆' };
...
const handleDelete = async (news) => {
    try {
        await ElMessageBox.confirm('确认删除该传统数字村落博物馆项目？', '提示', { type: 'warning' });
