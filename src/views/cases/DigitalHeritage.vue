<template>
    <div class="company-news">
        ...
        <div class="operation-bar">
            <el-button type="primary" @click="handleAdd">添加新的数字化文物</el-button>
        </div>
            ...
            <el-card v-for="news, index in newsList" :key="news.id" class="news-item">
                <template #header>
                    <div class="card-header">
                        <span style="font-weight: bolder;">第 {{ index + 1 }} 个文物数字化项目</span>
                        <div class="card-div">
                ...
                <div class="news-content">
                    <img v-if="news.imageUrl" :src="news.imageUrl" class="news-image" alt="文物数字化项目" />
        ...
        <!-- 编辑/添加对话框 -->
        <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑数字化文物项目' : '新增数字化文物项目'">
            <el-form :model="form" :rules="rules" ref="formRef">
                <el-form-item label="数字化文物的简要描述" prop="caseIntroduction">
...
// 验证规则
const rules = {
    caseIntroduction: [{ required: true, message: '请输入数字化文物简要描述', trigger: 'blur' }],
}
...
const fetchNews = async () => {
    try {
        ...
        // 过滤出 type 为 "文物数字化" 的项目
        newsList.value = data.data.filter(item => item.type === '文物数字化').reverse();
        console.log(newsList.value)
    } catch (error) {
        ElMessage.error('获取数字化文物列表失败');
    }
}
...
const submitForm = async () => {
    ...
    // 用户对象
    let parameterAdd = { caseIntroduction: form.value.caseIntroduction,type: '文物数字化' };
    let parameterEdit = {id:form.value.id, imageUrl: form.value.baseImageURL,caseIntroduction: form.value.caseIntroduction,type: '文物数字化' };
...
const handleDelete = async (news) => {
    try {
        await ElMessageBox.confirm('确认删除该数字化文物项目？', '提示', { type: 'warning' });
